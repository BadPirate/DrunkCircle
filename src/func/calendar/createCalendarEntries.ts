...
export async function createCalendarEntries(kennelID: string, limit: number = GoogleLimit): Promise<ProgressResult> {
...
query GQLRefreshKennelAddCount($kennelID: Int) {
  kennels(where: {id: {_eq: $kennelID}, google_refresh: {_is_null: false}, google_calendar: {_is_null: false}, google_token: {_is_null: false}}) {
    ...
    name
    short_name
    trails_aggregate(where: {start: {_is_null: false}, google_calendar: {_is_null: true}}) {
      aggregate {
        count
      }
    }
...
${trail.hares.map(hare => {
        return hare.hasherInfo.name;
      }).join(', ')}`;
    }
    ...
    await apiBackOff(`${trail.id}`, cal.events.insert({
      ...
      const id = r.data.id;
      if (!id)
        throw Error(`Unable to create ${trail.id}`);
      return id;
    }).then(gid => {
      return ac.mutate<GQLUpdateTrailGID>({
        mutation: gql`
